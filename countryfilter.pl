...
sub printrule {
	...
	if ($FilterType == 1) { # iptables
		...
		print "routing ip add filter name=\"������ ���ꥢ��³\" filtertype=INPUT srcaddr=$Address srcmask=$SubnetMaskVal1.$SubnetMaskVal2.$SubnetMaskVal3.$SubnetMaskVal4 dstaddr=0.0.0.0 dstmask=0.0.0.0 proto=any\n";
	} elsif ($FilterType ==4 ) { # text
		print "$Address/$SubnetBit\n"
	}
}
...
if ($FilterType == 1) { # iptables
	$TargetCommand=`which iptables`;
	print "IPTABLES=$TargetCommand\n";
	print "FILTERNAME=CKFILTER\n";
	print "TARGET=CKFILTERED\n";
} elsif ($FilterType == 2) { # ipfw
  $TargetCommand=`which ipfw`;
	print "IPFW=$TargetCommand\n";
	print "RULENUM=10000\n";
	print "ACTION=deny\n";
} elsif ($FilterType == 3) { # windows
...
if ($FilterType == 3) { # windows
	print "routing ip set filter name=\"������ ���ꥢ��³\" filtertype=INPUT action=FORWARD\n";
	print "commit\n";
}
